locals {
  ...
  subnet_ipv4_divisions = [for subnet_name in local.subnet_names : { name = subnet_name, new_bits = var.subnet_ipv4_cidr_shift }]
  subnet_ipv6_divisions = [for subnet_name in local.subnet_names : { name = subnet_name, new_bits = 8 }]
}
